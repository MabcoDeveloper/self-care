...
function ListProduct ()
{
  ...
  const { products, currency, axios, getToken, fetchProducts } = UseAppContext()
      ...
      <div className='flex flex-col gap-2 lg:w-11/12'>
        ...
        {products.map((pro) => (
          <div key={pro._id} className='grid grid-cols-[1fr_3.5fr_1.5fr_1.5fr_1fr] items-center gap-2 p-2 bg-white rounded-lg'>
            ...
            <div className='text-sm font-semibold'>From {currency}{(pro.price && (pro.price[pro.size?.[0]] ?? pro.price.get?.(pro.size?.[0]))) || 0}</div>
            <div>
              <label className='relative inline-flex items-flex items-center cursor-pointer text-gray-900 gap-3'>
                <input onClick={() => toggleStock(pro._id, !pro.inStock)} type="checkbox" className='sr-only peer' defaultChecked={pro.inStock} />
